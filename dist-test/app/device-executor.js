import { BadConnectionError, CommandTimeoutError, DeviceBusyError, ModbusError, ParseError, } from "@bluetooth/errors.js";
/**
...
export class DeviceCommandRunner {
    ...
    isStopRequested;
    sleep;
    constructor(manager, bus, getTelemetry, enqueueDeviceWork, isStopRequested, sleep) {
        this.manager = manager;
        this.bus = bus;
        this.getTelemetry = getTelemetry;
        this.enqueueDeviceWork = enqueueDeviceWork;
     ...
     */
    async runPackCommands(address, device, pollingState) {
     ...
     */
    async runCommandSet(address, device, commands, pollingState) {
     ...
     */
    async executeReadCommand(address, device, command) {
     ...
     */
    async trySelectPack(address, device, pack, pollingState) {
        try {
            await this.enqueueDeviceWork(address, async () => {
