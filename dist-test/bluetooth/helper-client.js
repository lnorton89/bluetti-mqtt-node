...
const PUBLISHED_HELPER_PATH = resolve(PACKAGE_ROOT, "artifacts", "helper", "win-x64", "BluettiMqtt.BluetoothHelper.exe");
/** Path to the helper .csproj for the `dotnet run` source fallback. */
const SOURCE_HELPER_PROJECT = resolve(PACKAGE_ROOT, "helper", "BluettiMqtt.BluetoothHelper", "BluettiMqtt.BluetoothHelper.csproj");
...
export class WindowsHelperClient {
     ...
     */
    constructor(command = resolveDefaultHelperCommand(), requestTimeoutMs = DEFAULT_REQUEST_TIMEOUT_MS) {
     ...
     */
    async scan(timeoutMs = 5_000) {
        const payload = await this.request("scan", { timeoutMs }, Math.max(this.requestTimeoutMs, timeoutMs + 5_000));
     ...
     */
    async readCharacteristic(sessionId, uuid) {
     ...
     */
    async writeCharacteristic(sessionId, uuid, data, withoutResponse = false) {
     ...
     */
    async subscribe(sessionId, uuid) {
     ...
     * accepts the line.
     */
    async request(command, argumentsObject, timeoutMs = this.requestTimeoutMs) {
        if (this.disposeRequested) {
        ...
        if (argumentsObject !== undefined) {
            request.arguments = argumentsObject;
        }
     ...
     */
    handleLine(line, onReady, onReadyError) {
        let message;
        try {
 ...
 */
function createHelperError(code, message) {
 ...
 */
function isRecoverableBluetoothConnectionError(code, message) {
 ...
 */
function resolveDefaultHelperCommand() {
 ...
 * ```
 */
export function createWindowsHelperRuntime(client = new WindowsHelperClient()) {
    return {
        discovery: client,
 ...
 */
function isHelperScanDevice(value) {
 ...
 */
function isHelperConnectPayload(value) {
    if (typeof value !== "object" || value === null) {
        return false;
    }
    const candidate = value;
 ...
 */
function isHelperNotificationEvent(message) {
