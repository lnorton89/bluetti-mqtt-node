...
import { UintField, BoolField, EnumField, DecimalField, DecimalArrayField, StringField, SwapStringField, VersionField, SerialNumberField } from "../dist/devices/field.js";
import { DeviceStruct } from "../dist/devices/struct.js";
import { BLUETTI_DEFINITIONS, BLUETTI_DEFINITION_MAP, BluettiDeviceModel } from "../dist/devices/definition.js";
import { createDeviceFromAdvertisement, isSupportedBluettiName } from "../dist/devices/registry.js";
...
function testAllDeviceStructsParse() {
  for (const def of BLUETTI_DEFINITIONS) {
    ...
    assert.ok(struct instanceof DeviceStruct, `${def.type} struct is a DeviceStruct`);
    ...
    for (const cmd of def.pollingCommands) {
      ...
      assert.ok(typeof parsed === "object", `${def.type} parse(${cmd.startingAddress}) produced object`);
    }
  }
}
...
function testBluettiDeviceModel() {
  ...
  const model = new BluettiDeviceModel("00:11:22:33:44:55", "2237000003358", ac500Def);
...
function testRegistryCreate() {
  const device = createDeviceFromAdvertisement("00:11:22:33:44:55", "AC5002237000003358");
...
function testRejectsUnknownDeviceName() {
  assert.throws(() => createDeviceFromAdvertisement("00:00:00:00:00:00", "UNKNOWN123"), /Unsupported Bluetti device/);
}
