...
function testRejectsUnknownField() {
	assert.throws(
		() => createTestDevice().buildSetterCommand("missing_field", 1),
		/not writable/,
	);
}
...
function testRejectsWrongBoolType() {
	assert.throws(
		() => createTestDevice().buildSetterCommand("charge_enabled", "ON"),
		/boolean value/,
	);
}
...
function testRejectsWrongIntegerType() {
	assert.throws(
		() => createTestDevice().buildSetterCommand("output_power", 7.5),
		/integer value/,
	);
}
...
function testRejectsUnknownEnumOption() {
	assert.throws(
		() => createTestDevice().buildSetterCommand("mode", "turbo"),
		/known enum option/,
	);
}
...
function testRejectsReadOnlyField() {
	assert.throws(
		() => createTestDevice().buildSetterCommand("read_only_status", 1),
		/not writable/,
	);
}
