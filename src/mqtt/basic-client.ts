...
export interface MqttClient {
	...
	subscribe(
		topic: string,
		onMessage: (message: ReceivedMqttMessage) => Promise<void> | void,
	): Promise<void>;
}
...
export class BasicMqttClient implements MqttClient {
	...
	private readonly callbacks = new Map<
		string,
		(message: ReceivedMqttMessage) => Promise<void> | void
	>();
	...
	async publish(message: PublishedMqttMessage): Promise<void> {
		await this.rawClient.publishAsync(message.topic, message.payload, {
