...
export class BluettiMqttBridge {
	...
	private messageListener: ((topic: string, payload: Buffer) => void) | null =
		null;
	...
	async run(): Promise<void> {
		...
		try {
			...
			this.logger.info("Subscribed to MQTT command topics", {
				topic: COMMAND_SUBSCRIPTION_TOPIC,
			});

			this.removeParserListener = this.bus.addParserListener(
				async (message) => {
					await this.handleParserMessage(message);
				},
			);
			this.messageListener = (topic, payload) => {
				void this.handleIncomingCommand(topic, new Uint8Array(payload)).catch(
					(error: unknown) => {
	 ...
	 */
	private async handleParserMessage(
		message: ParserMessage<BluettiDevice>,
	): Promise<void> {
		...
		for (const [name, value] of Object.entries(message.parsed)) {
			await client.publish(
				`${STATE_TOPIC_PREFIX}${message.device.type}-${message.device.serialNumber}/${name}`,
	 ...
	 */
	private async handleIncomingCommand(
		topic: string,
		payload: Uint8Array,
	): Promise<void> {
		...
		await this.bus.publishCommandMessage({ device, command } as CommandMessage<
			BluettiDevice,
			DeviceCommand
		>);
	}
