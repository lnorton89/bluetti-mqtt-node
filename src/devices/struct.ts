...
export class DeviceStruct {
	...
	addEnumField<TEnum extends EnumDefinition>(
		name: string,
		address: number,
	...
	parse(startingAddress: number, data: Uint8Array): ParsedFieldMap {
		if (data.length % 2 !== 0) {
			throw new RangeError(
				`Register data length must be even, got ${data.length}`,
			);
		}
		...
		for (const field of this.fields) {
			...
			if (
				field.address < registerRangeStart ||
				fieldEndExclusive > registerRangeEndExclusive
			) {
				continue;
			}
